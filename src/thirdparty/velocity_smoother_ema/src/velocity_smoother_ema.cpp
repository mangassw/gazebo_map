...
{
    nh_.param<double>("/velocity_smoother_ema/alpha_v", alpha_v, 0.4);
    nh_.param<double>("/velocity_smoother_ema/alpha_w", alpha_w, 0.4);
    nh_.param<std::string>("/raw_cmd_topic", raw_cmd_topic, "raw_cmd_vel");
    nh_.param<std::string>("/cmd_topic", cmd_topic, "cmd_vel");
    nh_.param<double>("/velocity_smoother_ema/cmd_rate", cmd_rate, 30.0);
    printf("vel_alpha_v is %f ,vel_alpha_v is %f \n",alpha_v,alpha_w);
