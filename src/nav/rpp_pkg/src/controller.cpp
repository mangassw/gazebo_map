                               ...
                               const geometry_msgs::PoseStamped& in_pose, geometry_msgs::PoseStamped& out_pose) const
{
  ...
  //716_1526改
   try {
        tf->transform(in_pose, out_pose, out_frame, ros::Duration(0.1)); // 设置超时时间为 0.1 秒
        out_pose.header.frame_id = out_frame;
...
double Controller::getLookAheadDistance(double vt)
{
  ...
  return rpp_pkg::math::clamp(lookahead_dist, min_lookahead_dist_, max_lookahead_dist_);
}
                                   ...
                                   geometry_msgs::PointStamped& pt, double& theta, double& kappa)
{
  ...
  else
  {
    ...
    rpp_pkg::vec2d::Vec2d prev_p(px - rx, py - ry);
    rpp_pkg::vec2d::Vec2d goal_p(gx - rx, gy - ry);
    std::vector<rpp_pkg::vec2d::Vec2d> i_points =
        rpp_pkg::math::circleSegmentIntersection(prev_p, goal_p, lookahead_dist);
    ...
    {
      rpp_pkg::vec2d::Vec2d p1(px, py), p2(gx, gy),
          p3(next_pose_it->pose.position.x, next_pose_it->pose.position.y);
      kappa = rpp_pkg::math::arcCenter(p1, p2, p3, false);
    }
    else
    {
      kappa = 0.0;
