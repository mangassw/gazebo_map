...
constexpr static const int PATH_SAFE_DIS_NUM = 1.3 / RECORD_PATH_LEN_DENS;
...
private:
  ...
  void gotoDone(const actionlib::SimpleClientGoalState& state, const mbf_msgs::MoveBaseResultConstPtr& result);
  void joyCtrlCB(const sensor_msgs::Joy::ConstPtr& joy);
  geometry_msgs::PoseStamped generateNewGoal(const geometry_msgs::PoseStamped& original_goal, double step = 0.05, double max_radius = 1.0);
  void serviceThread();
  bool navServiceCB(xj_robot_pnc::nav_goal::Request& req, xj_robot_pnc::nav_goal::Response& res);
  void navigate_home();
  void dockingStateCB(const std_msgs::String::ConstPtr& msg);
  void startDocking();
  void stopDocking();
...
private:
  ...
  bool docking_finished_ = false;
  bool nav_to_pre_dock_pos_ = false;
  ...
  ros::Publisher vision;
  ros::Publisher docking_ctrl_pub_;

  ros::Subscriber keyboard_sub_;
  ros::Subscriber costmap_sub_;
  ros::Subscriber costmap_update_sub_;
  ros::Subscriber joy_sub_;
  ros::Subscriber docking_state_sub_;
};
}