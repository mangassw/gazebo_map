...
void JzBridge::init() {
  ...
  control_sub_ = nh.subscribe(CommandTopic, 1, &JzBridge::control_callback, this);
  feedback_sub_ = nh.subscribe(FeedbackTopic, 1, &JzBridge::feedback_callback, this, ros::TransportHints().tcpNoDelay());
  fusion_analysis_timer_ = nh.createTimer(ros::Duration(TimerDuration), &JzBridge::timer_callback, this);
}

void JzBridge::control_callback(const geometry_msgs::Twist::ConstPtr& msg) {
...
void JzBridge::feedback_callback(const nav_msgs::Odometry::ConstPtr& msg) {
...
void JzBridge::timer_callback(const ros::TimerEvent& e) {
